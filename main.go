...
func main() {
	...
	var tableName string = "arms_probability"
	var fileName string = conf.Setting.FilePath + "arms_probability.xlsx"
	...
	for i := 1; i < rowsnum; i++ {
		...
		go func(i int) {
				...
				if v[0] == ":other" {
					...
					if len(v) > 1 {
						...
						case "unique":
							...
							if has > 0 {
								fmt.Println("[" + strconv.Itoa(<-ch) + "/" + strconv.Itoa(rowsnum-1) + "]" + v[0] + ":" + r.Value[v[0]] + "重複,自動跳過\n")
								sign <- "error"
								return
							}
						case "password":
							...
							if len(tmpvalue) == 2 {
								if []byte(tmpvalue[1])[0] == ':' {
									if _, ok := r.Value[string([]byte(tmpvalue[1])[1:])]; ok {
										...
										fmt.Println("[" + strconv.Itoa(<-ch) + "/" + strconv.Itoa(rowsnum-1) + "]密碼鹽" + string([]byte(tmpvalue[1])[1:]) + "字段不存在，自動跳過\n")
										sign <- "error"
										return
									}
								} else {
						...
						case "find":
							...
							if (*result)["id"] == "" {
								fmt.Print("[" + strconv.Itoa(<-ch) + "/" + strconv.Itoa(rowsnum-1) + "]表" + v[2] + " 中沒有找到 " + v[4] + " 為 " + r.Value[v[0]] + " 的數據,自動跳過\n")
								sign <- "error"
								return
							}
			...
			err = tx.Commit()
			base.Checkerr(err)
			fmt.Println("[" + strconv.Itoa(<-ch) + "/" + strconv.Itoa(rowsnum-1) + "]導入數據成功")
			sign <- "success"
		}(i)
	}
	for i := 1; i < rowsnum; i++ {
